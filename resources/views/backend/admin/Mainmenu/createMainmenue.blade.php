                  ...
                  @if(session('success'))
                   ...
                   @if(isset($edit_f))
                    ...
                    @csrf 
                        ...
                        <input type="hidden" id="sbu_user_edit" name="sbu_user_edit" value="{{$keydata->sbu_type ?? ''}}">
                        <div class="row mb-3">
                          <div class="card-body text-primary">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input sbu_id" type="radio" name="sbu_id" id="main_dashboard" rel="1" value="1"  @if(isset($edit_f)){{ ($keydata->viewer_id <= 1)   ? 'checked' : '' }}  @else checked @endif>
                                    <label class="form-check-label" for="inlineRadio1">Main dashboard</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input sbu_id" type="radio" name="sbu_id" id="sbu_dashboard" rel="2" value="2" @if(isset($edit_f)){{ ($keydata->viewer_id > 1)  ? 'checked' : '' }}  @endif>
                                ...
                                <select style="display: none;" class="usertype_div form-control_edited" id="sbu_user" name="sbu_user">
                                            ...
                                            @foreach($user_s as $user_s_id)
                                                    <option value="{{$user_s_id->id}}" {{ (old('sbu_user', isset($keydata->sbu_type) ? $keydata->sbu_type :old('sbu_user'))==$user_s_id->id) ? 'selected':'' }}>{{$user_s_id->title}}</option>
                                            @endforeach
                                </select>
                            </div> <!--card end-->  
...
$( "#ord_num" ).on( "keyup", function() {
    ...
    // var viewer_id=$(".sbu_id").attr('rel');
    var viewer_id=$('input[name="sbu_id"]:checked').val();
    if(viewer_id==2)
    {
        var sbutype_id=$("#sbu_user").val();
    }else{
        var sbutype_id=0;
    }
