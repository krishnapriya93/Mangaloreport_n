                            ...
                            <div class="row mb-3 card-header card-main">
                                ...
                                @endphp
                                    ...
                                    @foreach ($keydata->bodsub as $bodsub)
                                        <div class="col-sm-12">
                                            <div
                                                class="row card-header card-custom-header bg-secondary text-white mb-3 mt-2">
                                                <div class="col-sm-6 mb-btm">
                                                    <input type="hidden" value="{{ $bodsub->languageid ?? '' }}"
                                                        id="sel_lang{{ $bodsub->languageid }}" name="sel_lang[]">
                                                    <label for="path" class="col-sm-5 col-form-label">Details in
                                                        {{ $bodsub->lang_sel->name }}</label>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-sm-6 mb-btm" id="div{{ $bodsub->id }}">
                                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Name in
                                                        {{ $bodsub->lang_sel->name }} <span class="redalert">
                                                            *</span></label>
                                                    <input id="name{{ $bodsub->id }}" type="text"
                                                        class="form-control @error('name') is-invalid @enderror"
                                                        name="name[]" value="{{ $bodsub->name ?? old('name.' . $i) }}"
                                                        required autocomplete="name"
                                                        placeholder="Enter name {{ $bodsub->name }} here" autofocus>
                                                </div><br />
                                                ...
                                                <div class="col-sm-6 mb-btm" id="div_description{{ $bodsub->id }}">
                                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Description in
                                                        {{ $bodsub->lang_sel->name }}</label>
                                                    <textarea id="description{{ $bodsub->id }}" class="form-control ckeditor @error('description') is-invalid @enderror"
                                                        name="description[]" value="{{ $keydata->description ?? old('description.' . $i) }}" autocomplete="language"
                                                        placeholder="Enter description in {{ $bodsub->name }} here" autofocus>{{ $bodsub->description }}</textarea>
                                                </div><br />
                                            </div>
                                            <div class="row">
                                                <div class="col-sm-12 mb-btm" id="div_des{{ $bodsub->id }}">
                                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Designation in
                                                        {{ $bodsub->lang_sel->name }} <span class="redalert">
                                                            *</span></label>
                                                    <input id="desig_id{{ $bodsub->id }}" type="text"
                                                        class="form-control @error('desig_id') is-invalid @enderror"
                                                        name="desig_id[]"
                                                        value="{{ $bodsub->desig_id ?? old('name.' . $i) }}" required
                                                        autocomplete="name"
                                                        placeholder="Enter name {{ $bodsub->name }} here" autofocus>
                                                </div><br />
                                            ...
                                            <div class="row">
                                                <div class="col-sm-6 mb-btm" id="div{{$bodsub->languageid}}">
                                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Alt in {{ $bodsub->lang_sel->name }} <span class="redalert"> *</span></label>
                                                    <input id="alt{{$bodsub->languageid}}" type="text" class="form-control @error('alt') is-invalid @enderror" name="alt[]" value="{{ $bodsub->alt ?? old('alt.'.$i)}}" required autocomplete="alt" placeholder="Enter {{ $bodsub->lang_sel->name }} alt  here" autofocus >
                                            </div><br/>
                                            </div>
                                ...
                                <div class="row div_lan1 mb-3">
                                    <div class="col-sm-6 mb-btm">
                                        <div class="form-group">
                                            <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Mobile Number
                                            </label>
                                            <input id="mobilenumber" type="text"
                                                class="form-control @error('mobilenumber') is-invalid @enderror"
                                                name="mobilenumber"
                                                value="{{ $keydata->mobilenumber ?? old('mobilenumber') }}"
                                                autocomplete="mobilenumber" placeholder="Enter Mobile number here"
                                                autofocus>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 mb-btm">
                                        <div class="form-group">
                                            ...
                                            <input id="officenumber" type="text"
                                                ...
                                                autocomplete="officenumber" placeholder="Enter Office Number here"
                                                autofocus>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 mb-btm">
                                        <div class="form-group">
                                            ...
                                            <input id="email" type="email"
                                                ...
                                                value="{{ $keydata->email ?? old('email') }}" required
                                                autocomplete="email" placeholder="Enter Email here" autofocus>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 mb-btm">
                                        <div class="form-group">
                                            ...
                                            <input id="photo" type="file"
                                                ...
                                                value="{{ $keydata->photo ?? old('photo') }}" autocomplete="photo"
                                                placeholder="Enter Photo here" autofocus>
                                        </div>
                                        <div class="col-sm-3 mb-btm" id="">
                                            <img id="preview-image-before-upload" rel=""
                                                class="preview-image-before-upload imgstamp"
                                                src="{{ asset('/assets/backend/uploads/bod/' . $keydata->photo) }}" alt="preview image">
                                        </div><br />
                                    </div>
                                    <div class="col-sm-6 mb-btm">
