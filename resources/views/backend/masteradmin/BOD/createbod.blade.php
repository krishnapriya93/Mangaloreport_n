                ...
                <div class="card-body">
                    ...
                    @endif
                        ...
                        <form id="formid" method="POST" action="{{ route('updateBOD') }}" enctype="multipart/form-data">
                            @else
                        <form id="formiid" method="POST" action="{{ route('storeBOD') }}" enctype="multipart/form-data">
                        @endif
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12 mb-btm" id="div_des{{$bodsub->id}}">
                                                <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Designation in {{$bodsub->lang_sel->name}} <span class="redalert"> *</span></label>
                                                <input id="desig_id{{$bodsub->id}}" type="text" class="form-control @error('desig_id') is-invalid @enderror" name="desig_id[]" value="{{ $bodsub->desig_id ?? old('name.'.$i)}}" required autocomplete="name" placeholder="Enter name {{$bodsub->name}} here" autofocus >
                                            </div><br/>
                                    ...
                                    <div class="row card-header card-custom-header bg-secondary text-white mb-3 mt-2">
                                        <div class="col-sm-6 mb-btm">
                                          <input type="hidden"  value="{{$langs->id ?? ''}}" id="sel_lang{{$langs->id}}" name="sel_lang[]">
                                            <label for="path" class="col-sm-5 col-form-label" >Details in {{$langs->name}} </label>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-6 mb-btm" id="div{{$langs->id}}">
                                                ...
                                                <input id="name{{$langs->id}}" type="text" class="form-control @error('name') is-invalid @enderror" name="name[]" value="{{ $bodsub->name ?? old('name.'.$i)}}" required autocomplete="name" placeholder="Enter name {{$langs->name}} here" autofocus >
                                            </div><br/>
                                            ...
                                            <div class="col-sm-6 mb-btm" id="div_description{{$langs->id}}">
                                                ...
                                                <textarea id="description{{$langs->id}}" class="form-control ckeditor @error('description') is-invalid @enderror" name="description[]" value="{{ $keydata->description ?? old('description.'.$i)}}" autocomplete="language" placeholder="Enter description in {{$langs->name}} here" autofocus ></textarea>
                                            </div><br/>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-12 mb-btm" id="div_des{{$langs->id}}">
                                                <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Designation in {{$langs->name}} <span class="redalert"> *</span></label>
                                                <input id="desig_id{{$langs->id}}" type="text" class="form-control @error('desig_id') is-invalid @enderror" name="desig_id[]" value="{{ $bodsub->desig_id ?? old('name.'.$i)}}" required autocomplete="name" placeholder="Enter name {{$langs->name}} here" autofocus >
                                            </div><br/>
