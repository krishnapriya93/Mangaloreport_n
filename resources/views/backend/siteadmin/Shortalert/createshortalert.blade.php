        ...
        <div class="col-md-12">
            <div class="card" id="entry_div" @if($errors->any()) style="display: inline;" @lese style="display: none;" @endif>
                ...
                <div class="card-body">
                    ...
                    @endif
                    @if(isset($edit_f))
                    <form id="formid" method="POST" action="{{ route('updateshortslert') }}" enctype="multipart/form-data">
                    @else
                    <form id="formid" method="POST" action="{{ route('storeshortslert') }}" enctype="multipart/form-data">
                    @endif
                    ...
                    @csrf 
                        ...
                        <div class="row mb-3 card-header card-main">
                            ...
                            @if(isset($keydata->id)) @foreach(($keydata->shortalert_sub) as $shortalert_sub) 
                            <input type="hidden"  value="{{$shortalert_sub->languageid ?? ''}}" id="sel_lang{{$shortalert_sub->languageid}}" name="sel_lang[]">
                            <div class="col-sm-12 card-header card-custom-header bg-secondary text-white mb-3"><label for="path" class="col-sm-2 col-form-label" >{{$shortalert_sub->name}}</label></div><br/>                  
                            <div class="row div_lan1 mb-3">
                                <div class="col-sm-12 mb-btm" id="div{{$shortalert_sub->id}}"> 
                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Main Title in {{$shortalert_sub->name}} <span class="redalert"> *</span></label>
                                    <input id="title{{$shortalert_sub->id}}" type="text" class="form-control @error('title') is-invalid @enderror" name="title[]" value="{{ $shortalert_sub->title ?? old('title.'.$i)}}" required autocomplete="title" placeholder="Enter main {{$shortalert_sub->name}} here" autofocus >
                                </div><br/>

                                <div class="col-sm-12 mb-btm" id="div_sub{{$shortalert_sub->id}}"> 
                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Sub title in {{$shortalert_sub->name}} <span class="redalert"> *</span></label>
                                   <textarea id="con_title{{$shortalert_sub->id}}" class="form-control ckeditor @error('con_title') is-invalid @enderror" name="con_title[]" value="{{ $shortalert_sub->title ?? old('con_title.'.$i)}}" required autocomplete="language" placeholder="Enter Content in {{$shortalert_sub->name}} here" autofocus > {{$shortalert_sub->content}}</textarea>
                                </div><br/>
                            ...
                            <div class="row">
                               <div class="col-sm-6 mb-btm"> 
                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Colour class<span class="redalert"> *</span></label>
                                    <input id="colorclass" type="color" class="form-control @error('colorclass') is-invalid @enderror" name="colorclass" value="{{ $keydata->colorclass ?? old('colorclass')}}" required autocomplete="colorclass" placeholder="Enter color class here" autofocus >
                                </div><br/>
                            </div><br/>
                            <div><br/></div>
                            <div class="row mb-1">
