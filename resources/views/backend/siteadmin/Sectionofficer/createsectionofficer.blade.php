    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card" id="entry_div" @if($errors->any()) style="display: inline;" @lese style="display: none;" @endif>
                <div class="card-header text-white card-header-main">{{isset($edit_f) ? 'Update' : 'Add'}} Section office</div>
                <div class="card-body">
                    @if(session('success'))
                    ...
                    @endif
                    @if(isset($edit_f))
                    <form id="formid" method="POST" action="{{ route('updatesectionoffice') }}" enctype="multipart/form-data">
                    @else
                    <form id="formid" method="POST" action="{{ route('storesectionoffice') }}" enctype="multipart/form-data">
                    @endif
